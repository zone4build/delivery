...
const AverageRatings: React.FC<AverageRatingsProps> = ({
  ...
  return (
    <div className="p-5 md:py-12 lg:px-16 bg-gray-50/50 rounded-2xl border border-gray-100 shadow-sm mx-5 lg:mx-16 mb-8">
      <h2 className="mb-7 text-xl font-bold tracking-tight text-heading">
        {t('text-ratings-reviews')} <span className="text-accent">{title}</span>
      </h2>
      ...
      <div className="flex w-full flex-col divide-y divide-gray-200 divide-opacity-70 sm:flex-row sm:items-center sm:space-x-8 sm:divide-y-0 sm:divide-x rtl:sm:space-x-reverse rtl:sm:divide-x-reverse">
        ...
        <div className="w-full space-y-3 py-0.5 pt-4 sm:w-auto sm:pt-0 ltr:sm:pl-8 rtl:sm:pr-8">
          <RatingProgressBar
            ...
            colorClassName="bg-gradient-to-r from-yellow-400 to-orange-500"
          />
          <RatingProgressBar
            ratingProgressItem={ratingCount.find(
            ...
            ratingId={4}
            totalReviews={totalReviews!}
            colorClassName="bg-gradient-to-r from-emerald-400 to-teal-600"
          />
          <RatingProgressBar
            ratingProgressItem={ratingCount.find(
            ...
            ratingId={3}
            totalReviews={totalReviews!}
            colorClassName="bg-gradient-to-r from-sky-400 to-blue-500"
          />
          <RatingProgressBar
            ratingProgressItem={ratingCount.find(
            ...
            ratingId={2}
            totalReviews={totalReviews!}
            colorClassName="bg-gradient-to-r from-amber-400 to-orange-400"
          />
          <RatingProgressBar
            ratingProgressItem={ratingCount.find(
            ...
            ratingId={1}
            totalReviews={totalReviews!}
            colorClassName="bg-gradient-to-r from-rose-400 to-red-600"
          />
        </div>
      </div>
    </div>
