...
export const AddressGrid: React.FC<AddressesProps> = ({
  ...
  useEffect(() => {
    if (addresses?.length) {
      if (selectedAddress?.id) {
        ...
        if (index > -1 && addresses[index]?.id !== selectedAddress?.id) {
  ...
  return (
    <div className={className}>
      <AddressHeader onAdd={disabled ? undefined : onAdd} count={count} label={label} />
      {!addresses?.length ? (
            ...
            {addresses?.map((address) => (
              <RadioGroup.Option value={address} key={address?.id}>
                {({ checked }: { checked: boolean }) => (
                  <AddressCard
                    checked={checked}
                    onDelete={disabled ? undefined : () => onDelete(address)}
                    onEdit={disabled ? undefined : () => onEdit(address)}
                    address={address}
                  />
                )}
              </RadioGroup.Option>
