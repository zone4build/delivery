...
export const ScheduleGrid: React.FC<ScheduleProps> = ({
  ...
  const schedules = settings?.deliveryTime || settings?.delivery_time;
  ...
  }, [schedules, setSchedule, selectedSchedule]);
  return (
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-28 animate-pulse rounded border border-border-200 bg-gray-100"
            />
          ))}
        </div>
      ) : schedules && schedules?.length ? (
        <RadioGroup value={selectedSchedule} onChange={setSchedule} by="title">
