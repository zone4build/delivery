  ...
  const { price: discountPrice } = usePrice(
    ...
    discount && {
      amount: coupon?.type === 'percentage' 
        ? base_amount * (Number(coupon.amount) / 100) 
        : Number(discount),
      currencyCode: items?.[0]?.currency,
    }
  );
  ...
  return (
    <div className={className}>
      ...
      <div className="mt-4 space-y-2">
        ...
        {verifiedResponse?.show_tax_breakdown !== false && (
      ...
      {/* Hide Wallet section as requested by user - Logic remains in background */}
      {/* {verifiedResponse && isAuthorized && (
        <Wallet
      ...
      {verifiedResponse && !isAuthorized && (
        <div className="mt-4 p-4 text-sm text-center bg-gray-100 rounded text-gray-500">
          Sign in to earn and spend wallet points!
        </div>
      )} */}
