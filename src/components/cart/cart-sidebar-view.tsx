...
const CartSidebarView = () => {
  ...
  function handleCheckout() {
    ...
    const shopSlug = resolveShopSlug() || items?.[0]?.shop_slug;
    if (isRegularCheckout) {
      router.push(shopRoute(shopSlug, Routes.checkout), undefined, {
        locale: language
      });
    } else {
      router.push(shopRoute(shopSlug, Routes.checkoutDigital), undefined, {
        locale: language
      });
    }
  ...
  return (
    <section className="relative flex h-full flex-col">
      ...
      {/* @ts-ignore */}
      <AnimateSharedLayout>
        <MotionDiv layout className="flex-grow pt-16 pb-20">
          {items.length > 0 ? (
      ...
      <footer className={`fixed ${router.asPath.includes('/map') ? 'bottom-0' : 'bottom-28'} z-10 w-full max-w-md bg-light py-5 px-6`}>
        <button
