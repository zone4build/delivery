            ...
            <div className="p-7">
              ...
              {shop?.settings?.businessHours && (
                <div className="flex flex-col mt-7">
                  <span className="mb-3 text-sm font-semibold text-heading uppercase tracking-wider">
                    {t('text-opening-hours', 'Opening Hours')}
                  </span>
                  <div className="space-y-2 bg-gray-50 p-4 rounded-lg">
                    {['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'].map((day) => {
                      const schedule = shop.settings.businessHours[day];
                      if (!schedule) return null;
                      return (
                        <div key={day} className="flex justify-between text-xs font-medium">
                          <span className="capitalize text-body">{day}</span>
                          <span className={schedule.isClosed ? 'text-red-500 italic' : 'text-heading'}>
                            {schedule.isClosed ? t('text-closed', 'Closed') : `${schedule.open} - ${schedule.close}`}
                          </span>
                        </div>
                      );
                    })}
