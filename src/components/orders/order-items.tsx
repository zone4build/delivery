...
const OrderItemList = (_: any, record: any) => {
  ...
  let name = typeof record.name === 'object' 
    ? (record.name[record.language || 'en'] || record.name['en']) 
    : record.name;
  ...
  if (record?.pivot?.variation_option_id) {
    ...
    const formattedVariationTitle = typeof variationTitle === 'object'
      ? (variationTitle[record.language || 'en'] || variationTitle['en'])
      : variationTitle;
    name = `${name} - ${formattedVariationTitle}`;
  }
  return (
...
export const OrderItems = ({
  ...
  const orderTableColumns = [
    ...
    },
    {
      ...
      align: alignRight,
      width: 100,
      render: function RenderPrice(pivot: any, record: any) {
  const routes = useShopRoutes();
        const { price } = usePrice({
