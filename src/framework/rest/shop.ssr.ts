...
export const getStaticPaths: GetStaticPaths<ParsedQueryParams> = async ({
  ...
  try {
    ...
    console.warn('API not available during build (shops), using fallback mode:', error);
    return {
      paths: [],
      fallback: 'blocking',
    };
...
> = async ({ params, locale }) => {
  ...
  try {
    ...
    console.error(`[Shop SSR] Error fetching shop data for slug "${slug}":`, error.message || error);
    return {
      redirect: {
        destination: '/map',
        permanent: false,
